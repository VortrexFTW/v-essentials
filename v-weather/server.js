...
bindEventHandler("OnResourceStart", thisResource, function (event, resource) {
	if (server.game == 10) {
		console.warn(`[${thisResource.name}] The v-weather resource is not available for Mafia: The City of Lost Heaven. Stopping resource ...`);
		thisResource.stop();
		return;
	}
});
...
bindEventHandler("OnResourceStart", thisResource, function (event, resource) {
	...
	if (scriptConfig.useRealWorldWeather) {
		console.log(`[${thisResource.name}] Resource started. Current weather location: ${scriptConfig.weatherLocation}`);
		setInterval(() => {
			updateWeather();
...
async function updateWeather() {
	...
	new Promise(resolve => {
		...
		if (typeof data.current == "undefined") {
			console.error(`[${thisResource.name}] Current weather data is undefined. Resource stopping ...`);
			thisResource.stop();
		}
		...
		if (typeof gtaWeatherData == "undefined") {
			console.error(`[${thisResource.name}] No GTA weather data found for API code: ${data.current.condition.code}. Weather not updated (existing in-game weather will remain).`);
			return false;
		}

		console.log(`[${thisResource.name}] Updating in-game weather to: ${gtaWeatherData.name} (${gtaWeatherData.weatherId})`);
		game.forceWeather(gtaWeatherData.weatherId);
	});
}
...
addCommandHandler("fallingsnow", (command, params, client) => {
	if (getPlayerAdminLevel(client) < serverConfig.commandLevels[command]) {
		message(`You can't use the ${command} command!`, COLOUR_ORANGE);
	}
...
addCommandHandler("groundsnow", (command, params, client) => {
	if (client.getData("v.admin") < serverConfig.commandLevels[command]) {
		message(`You can't use the ${command} command!`, COLOUR_ORANGE);
	}
...
function getPlayerAdminLevel(client) {
	return client.getData("v.admin") || 0;
}
